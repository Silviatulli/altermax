...
#to inherit node, node should be defined
...
class Node(object):
    def __init__(self, depth, parent):
        self.parent = parent
        self.depth = depth
        optimal_score = None # highest/lowest score that can be achieved by the current player - I would put this get_score
    
    def score(is_child_turn, isFinished):
        if is_child_turn and isFinished():
            return 1
        else:
            return -1
        return 0
...
#we need to define isTerminal
def minimax(node, depth, maximizingPlayer, isTerminal):
